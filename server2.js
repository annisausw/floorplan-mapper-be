...
app.post("/api/maps/:id/vertices", authenticateToken, (req, res) => {
  ...
  // FIXED: We pull the original `id` from `req.body` directly just in case
  // Zod strictly stripped it out of `parsed.data` for not being explicitly in the schema.
  parsed.data.forEach((v, index) => {
    const providedId = req.body[index].id;
    const newVertex = { ...v, id: providedId || crypto.randomUUID() };
